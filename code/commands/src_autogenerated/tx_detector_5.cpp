...
namespace mzn {
TxDetector5::TxDetector5():
    Command(0, 3),
    detector_number_to_use_as_base(),
    detector_number_of_this_invocation(),
    detector_options() { }

uint16_t TxDetector5::msg_to_data(std::vector<uint8_t> const & msg,
                                  uint16_t mf_begin) {
...
uint16_t TxDetector5::data_to_msg(std::vector<uint8_t> & msg,
...
std::ostream & TxDetector5::os_print(std::ostream & cmd_os) const {
    cmd_os << "\n --- TX_DETECTOR_5 ---  \n";
