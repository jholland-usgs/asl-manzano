...
namespace mzn {
TxDetector7::TxDetector7():
    Command(0, 3),
    detector_number_to_use_as_base(),
    detector_number_of_this_invocation(),
    detector_options() { }

uint16_t TxDetector7::msg_to_data(std::vector<uint8_t> const & msg,
                                  uint16_t mf_begin) {
...
uint16_t TxDetector7::data_to_msg(std::vector<uint8_t> & msg,
...
std::ostream & TxDetector7::os_print(std::ostream & cmd_os) const {
    cmd_os << "\n --- TX_DETECTOR_7 ---  \n";
