...
namespace mzn {
T1DataSubscriptionServerParameter::T1DataSubscriptionServerParameter():
    Command(4, 40),
...
uint16_t T1DataSubscriptionServerParameter::msg_to_data(std::vector<uint8_t> const & msg,
                                                        uint16_t mf_begin) {

    if ( msg.size()  < cmd_data_size_ + mf_begin) {
        throw FatalException(
            "T1DataSubscriptionServerParameter",
            "msg_to_data",
...
uint16_t T1DataSubscriptionServerParameter::data_to_msg(std::vector<uint8_t> & msg,
...
std::ostream & T1DataSubscriptionServerParameter::os_print(std::ostream & cmd_os) const {
    cmd_os << "\n --- T1_DATA_SUBSCRIPTION_SERVER_PARAMETER ---  \n";
