...
namespace mzn {
T1ClockProcessing::T1ClockProcessing():
    Command(6, 16),
    timezone_offset(),
...
uint16_t T1ClockProcessing::msg_to_data(std::vector<uint8_t> const & msg,
                                        uint16_t mf_begin) {
...
uint16_t T1ClockProcessing::data_to_msg(std::vector<uint8_t> & msg,
...
std::ostream & T1ClockProcessing::os_print(std::ostream & cmd_os) const {
    cmd_os << "\n --- T1_CLOCK_PROCESSING ---  \n";
